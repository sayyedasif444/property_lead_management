  ...
  return (
    ...
    >
      <List
        ...
        className='pb-2'
        dataSource={
          currentData.hasOwnProperty('deduction')
            ? currentData.deduction.length > 0
              ? JSON.parse(currentData.deduction[0].amount).map((ele) => {
                ...
                onClick={(e) => {
                  ...
                  addSalaryAddon({
                    amount: JSON.stringify(
                      JSON.parse(currentData.deduction[0].amount).filter(
                        (ele, ind) => ind !== index
                      )
                    ),
                    effective_date: currentData.deduction[0].effective_date,
                    type: 'Deduction',
