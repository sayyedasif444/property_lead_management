...
import { deleteRepayment } from '../../../../apis/dashboard/Customer';
...
const Index = ({
  ...
  useEffect(() => {
    if (data.customerRepayments !== null) {
      var result = data.customerRepayments;
      var dataset = [];
      let amount = 0;
          ...
          action: (
            <div>
              ...
              <Popconfirm
                title='Are you sure?'
                onConfirm={(e) => {
                  deleteRepayment({ id: element.id, customer_id: data.id });
                }}
                okText='Yes'
                cancelText='No'
              >
      ...
      <Row>
        <Col sm={24} md={24} lg={24}>
          <Table
            ...
            pagination={false}
          />
          <h5 className='pl-2 pt-2'>Total Refund: {amount}</h5>
        </Col>
      </Row>
...
export default connect(mapStateToProps, { deleteRepayment })(Index);
